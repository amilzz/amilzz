...
## Test admixture for Walker Lake

How admixture done here....
...
bamlist<-read_csv("bamlists/walrbt.bamlist", col_names=c("Path"))
...
write_csv(meta, "meta/meta-wal-rbt.csv" )
```

```{r}
## Get qopt
q2<-read_delim("outputs/100/walrbt_stru_k2.qopt", delim=" ", col_names=FALSE) %>% dplyr::select(X1, X2) %>%
  rename(Q1=X1, Q2=X2)
...
#  theme(axis.text.x = element_blank(), axis.ticks.x=element_blank()) +
  scale_x_continuous(breaks=pops2$Position, labels=pops2$MinorBasin) +
  theme(axis.text.x=element_text(angle=45, vjust=0, hjust=.5, size=6))


p2

ggsave("outputs/100/wal-rbt-k2.jpeg", width=8, height=4)
```
